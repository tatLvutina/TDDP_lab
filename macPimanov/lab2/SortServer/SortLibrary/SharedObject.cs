...
namespace SortLibrary
{
    public class SharedObject : MarshalByRefObject
    {
        ...
        void GenerateTasks()
        {
            ...
            int step = dataCount / tasksCount; // Разделение массива на каждую задачу
        ...
        {
            //Проверка длины массива, если она равна 1, то возвращаем массив, так как он не нуждается в сортировке
            if (buff.Length > 1)
            {
                //Массивы для хранения половин входящего буфера
                int[] left = new int[buff.Length / 2];
                //Для проверки ошибки некорректного разбиения массива, в случае, если длина - непарное число
                ...
                //Если длина субмассивов больше единицы, то мы повторно рекурсивным методом вызываем функцию разбиения массива
                if (left.Length > 1)
                    left = Sort(left);
                if (right.Length > 1)
                    right = Sort(right);
                //Сортировка слиянием половин
